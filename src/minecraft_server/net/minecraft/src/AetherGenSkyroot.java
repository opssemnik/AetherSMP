...
public class AetherGenSkyroot extends WorldGenerator
{
    ...
    public boolean generate(World world, Random random, int i, int j, int k)
    {
            ...
            for(int i2 = i - byte0; i2 <= i + byte0 && flag; i2++)
            {
                for(int l2 = k - byte0; l2 <= k + byte0 && flag; l2++)
                {
                    if(i1 >= 0 && i1 < 128)
                    {
                        ...
                        if(j3 != 0 && j3 != AetherBlocks.SkyrootLeaves.blockID)
                        {
                            flag = false;
                        }
                    } else
        ...
        for(int k1 = (j - 3) + l; k1 <= j + l; k1++)
        {
            ...
            for(int k3 = i - i3; k3 <= i + i3; k3++)
            {
                ...
                for(int i4 = k - i3; i4 <= k + i3; i4++)
                {
                    ...
                    {
                        world.setBlock(k3, k1, i4, AetherBlocks.SkyrootLeaves.blockID);
                    }
                }
        ...
        for(int l1 = 0; l1 < l; l1++)
        {
            ...
            {
                world.setBlockAndMetadata(i, j + l1, k, AetherBlocks.Log.blockID, 0);
            }
        }
