...
//            WorldGenerator, World, Material, AetherBlocks, 
//            Block, EnumSkyBlock
    ...
    public boolean generate(World world, Random random, int i, int j, int k)
    {
        ...
        for(int k1 = 0; k1 < 16; k1++)
        {
            for(int k2 = 0; k2 < 16; k2++)
            {
                for(int k3 = 0; k3 < 8; k3++)
                {
                    ...
                    {
                        world.setBlock(i + k1, j + k3, k + k2, k3 >= 4 ? 0 : field_15235_a);
                    }
                }
        ...
        for(int l1 = 0; l1 < 16; l1++)
        {
            for(int l2 = 0; l2 < 16; l2++)
            {
                ...
                {
                    if(aflag[(l1 * 16 + l2) * 8 + l3] && world.getBlockId(i + l1, (j + l3) - 1, k + l2) == AetherBlocks.Dirt.blockID && world.getSavedLightValue(EnumSkyBlock.Sky, i + l1, j + l3, k + l2) > 0)
                    {
                        world.setBlock(i + l1, (j + l3) - 1, k + l2, AetherBlocks.Grass.blockID);
                    }
                }
            ...
            for(int i2 = 0; i2 < 16; i2++)
            {
                for(int i3 = 0; i3 < 16; i3++)
                {
                    for(int i4 = 0; i4 < 8; i4++)
                    {
                        ...
                        {
                            world.setBlockAndMetadata(i + i2, j + i4, k + i3, AetherBlocks.Holystone.blockID, 0);
                        }
                    }
