...
//            Teleporter, Entity, MathHelper, World, 
//            AetherBlocks, Block, Material
    ...
    public boolean func_4106_b(World world, Entity entity)
    {
        ...
        for(int j1 = l - c; j1 <= l + c; j1++)
        {
            ...
            for(int j2 = i1 - c; j2 <= i1 + c; j2++)
            {
                ...
                {
                    if(world.getBlockId(j1, k2, j2) != AetherBlocks.Portal.blockID)
                    {
                        continue;
                    }
                    for(; world.getBlockId(j1, k2 - 1, j2) == AetherBlocks.Portal.blockID; k2--) { }
        ...
        if(d >= 0.0D)
        {
            ...
            if(world.getBlockId(k1 - 1, l1, i2) == AetherBlocks.Portal.blockID)
            {
                d2 -= 0.5D;
            }
            if(world.getBlockId(k1 + 1, l1, i2) == AetherBlocks.Portal.blockID)
            ...
            if(world.getBlockId(k1, l1, i2 - 1) == AetherBlocks.Portal.blockID)
            {
                d6 -= 0.5D;
            }
            if(world.getBlockId(k1, l1, i2 + 1) == AetherBlocks.Portal.blockID)
                        ...
                        for(int j9 = 0; j9 < 3; j9++)
                        {
                            for(int k10 = 0; k10 < 4; k10++)
                            {
                                for(int l11 = -1; l11 < 4; l11++)
                                {
                                    ...
                                    if(l11 < 0 && !blockIsGood(world.getBlockId(j12, l12, j13), world.getBlockMetadata(j12, l12, j13)) || l11 >= 0 && !world.isAirBlock(j12, l12, j13))
                                    {
                                        break label0;
                                    }
                                }
            ...
            for(int j2 = i - byte0; j2 <= i + byte0; j2++)
            {
                ...
                for(int k3 = k - byte0; k3 <= k + byte0; k3++)
                {
                    ...
                    for(int l4 = 127; l4 >= 0; l4--)
                    {
                        ...
                        for(; world.isAirBlock(j2, l4 - 1, k3) && l4 > 0; l4--) { }
label1:
                        for(int l5 = l1; l5 < l1 + 2; l5++)
                        {
                            ...
                            for(int k9 = 0; k9 < 4; k9++)
                            {
                                for(int l10 = -1; l10 < 4; l10++)
                                {
                                    ...
                                    int k12 = l4 + l10;
                                    int i13 = k3 + (k9 - 1) * j8;
                                    if(l10 < 0 && !blockIsGood(world.getBlockId(i12, k12, i13), world.getBlockMetadata(i12, k12, i13)) || l10 >= 0 && !world.isAirBlock(i12, k12, i13))
                                    {
                                        break label1;
                                    }
                                }
            ...
            for(int i5 = -1; i5 <= 1; i5++)
            {
                for(int i6 = 1; i6 < 3; i6++)
                {
                    for(int j7 = -1; j7 < 3; j7++)
                    {
                        ...
                        boolean flag = j7 < 0;
                        world.setBlockWithNotify(k8, l9, i11, flag ? Block.glowStone.blockID : 0);
                    }
        ...
        for(int j5 = 0; j5 < 4; j5++)
        {
            ...
            for(int j6 = 0; j6 < 4; j6++)
            {
                for(int k7 = -1; k7 < 4; k7++)
                {
                    ...
                    world.setBlockWithNotify(l8, i10, j11, flag1 ? Block.glowStone.blockID : AetherBlocks.Portal.blockID);
                }
    ...
    public boolean blockIsGood(int i, int j)
    {
        if(i == 0)
        {
            return false;
        }
        if(!Block.blocksList[i].blockMaterial.isSolid())
        {
            return false;
        }
        return i != AetherBlocks.Aercloud.blockID || j != 0;
    }
