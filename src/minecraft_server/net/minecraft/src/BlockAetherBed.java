...
//            ChunkCoordinates, EnumStatus, BlockBed, ModelBed, 
//            Item, IBlockAccess
...
public class BlockAetherBed extends Block
{
    ...
    public boolean blockActivated(World world, int i, int j, int k, EntityPlayer entityplayer)
    {
        ...
        EnumStatus enumstatus = sleepInBedAt(entityplayer, i, j, k);
    ...
    public EnumStatus sleepInBedAt(EntityPlayer entityplayer, int i, int j, int k)
    {
        World world = entityplayer.worldObj;
        if(!world.singleplayerWorld)
        {
            if(entityplayer.func_22057_E() || !entityplayer.isEntityAlive())
            {
            ...
            if(Math.abs(entityplayer.posX - (double)i) > 3D || Math.abs(entityplayer.posY - (double)j) > 2D || Math.abs(entityplayer.posZ - (double)k) > 3D)
            {
                return EnumStatus.TOO_FAR_AWAY;
            }
        }
        entityplayer.setSize(0.2F, 0.2F);
        entityplayer.yOffset = 0.2F;
        if(world.blockExists(i, j, k))
        {
            ...
            func_22052_e(entityplayer, i1);
            entityplayer.setPosition((float)i + f, (float)j + 0.9375F, (float)k + f1);
        } else
        {
            entityplayer.setPosition((float)i + 0.5F, (float)j + 0.9375F, (float)k + 0.5F);
        }
    ...
    private void func_22052_e(EntityPlayer entityplayer, int i)
    {
        entityplayer.field_22066_z = 0.0F;
        entityplayer.field_22067_A = 0.0F;
        switch(i)
        {
        case 0: // '\0'
            entityplayer.field_22067_A = -1.8F;
            break;
        ...
        case 1: // '\001'
            entityplayer.field_22066_z = 1.8F;
            break;
