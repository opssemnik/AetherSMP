...
public class PlayerBaseAether extends PlayerBase
{
    ...
    public PlayerBaseAether(EntityPlayerMP entityplayersp)
    {
        super(entityplayersp);
        maxHealth = 20;
        inv = new InventoryAether(player);
        player.personalCraftingInventory = new ContainerAether(player.inventory, inv, !player.worldObj.singleplayerWorld);
        player.inventory = new InventoryAether(entityplayersp);
        readCustomData();
    }
    ...
    { /*
        ...
        try
        {
            File file = new File(this.player.get, "aether.dat");
