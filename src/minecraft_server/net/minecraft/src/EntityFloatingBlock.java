...
//            Entity, MathHelper, World, AetherBlocks, 
//            Block, AxisAlignedBB, EntityFallingSand, BlockFloating, 
//            NBTTagCompound
...
public class EntityFloatingBlock extends Entity
{
    ...
    public void onUpdate()
    {
        ...
        if(worldObj.getBlockId(i, j, k) == blockID || worldObj.getBlockId(i, j, k) == AetherBlocks.Grass.blockID && blockID == AetherBlocks.Dirt.blockID)
        {
        ...
        if(isCollidedVertically && !onGround)
        {
            ...
            motionY *= -0.5D;
            setEntityDead();
            if(worldObj.canBlockBePlacedAt(blockID, i, j, k, true, 1) && !BlockFloating.canFallAbove(worldObj, i, j + 1, k) && worldObj.setBlockAndMetadataWithNotify(i, j, k, blockID, metadata) || worldObj.singleplayerWorld);
        } else
        if(flytime > 100 && !worldObj.singleplayerWorld)
        {
            setEntityDead();
        }
    }
