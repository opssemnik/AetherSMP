...
public class EntityFlamingArrow extends Entity
{
    ...
    public void onUpdate()
    {
        ...
        worldObj.spawnParticle(rand.nextBoolean() ? "flame" : "smoke", posX, posY, posZ, 0.0D, 0.0D, 0.0D);
        ticksInAir++;
        ...
        if(movingobjectposition != null)
        {
            if(movingobjectposition.entityHit != null)
            {
                if(movingobjectposition.entityHit.attackEntityFrom(owner, 4))
                {
                    ...
                    int i1 = MathHelper.floor_double(movingobjectposition.entityHit.boundingBox.minX);
                    int j1 = MathHelper.floor_double(movingobjectposition.entityHit.boundingBox.minY);
                    int l1 = MathHelper.floor_double(movingobjectposition.entityHit.boundingBox.minZ);
                    worldObj.setBlockWithNotify(i1, j1, l1, 51);
                    setEntityDead();
                } else
                {
            ...
            } else
            {
                ...
                worldObj.setBlockWithNotify(k1, i2, j2, 51);
                inGround = true;
                arrowShake = 7;
            }
        }
